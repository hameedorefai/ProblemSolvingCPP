...
void PrintGenerateKeys( int NumberOfKeys,enCharTybe CharacterTybe)
{
	int KeysCounter=1;
	while (NumberOfKeys--) {
		
		cout <<"Key ["<<KeysCounter<<"] : "<< GetGenrateKey(CharacterTybe) << endl;
		KeysCounter++;
	}
}
...
int main() {
	...
	enCharTybe CharacterTybe = enCharTybe::Digit;
	int NumberOfKeys = ReadPositiveNumber("Enter a Positive number :");
	PrintGenerateKeys(NumberOfKeys, CharacterTybe);
