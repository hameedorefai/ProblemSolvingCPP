...
enum eFormatDate {
	ByName, All, DMYdash, DMYslash, YDMslash, MDYslash, MDYdash
	// DMY = Day Month Year, slash = / , dash = -
};
struct stDate
{
	int Year;
...
{
	// ?? ??? ?????? ???? ?? ??? ???? ???????? ??????? ?? ?? 
	return (IsDate1MoreThanDate2(Period1.EndDate, Period2.StartDate) || IsDate1EqualsDate2(Period1.EndDate, Period2.StartDate))
		&& (IsDate1MoreThanDate2(Period2.EndDate, Period1.StartDate) || IsDate1EqualsDate2(Period2.EndDate, Period1.StartDate));
	/*return IsDate1MoreThanDate2(Period1.EndDate, Period2.StartDate)
		&& IsDate1MoreThanDate2(Period2.EndDate, Period1.StartDate);*/

}
bool IsDateInPeriod(stPeriod Period, stDate Date)
{
	return (
		(IsDate1LessThanDate2(Period.StartDate, Date)
			|| IsDate1EqualsDate2(Period.StartDate, Date))
		&&
		(IsDate1MoreThanDate2(Period.EndDate, Date)
			|| IsDate1EqualsDate2(Period.EndDate, Date))
		);
	// ??? ????? ???????? ??????? ??????? ??Equals
	return !(IsDate1LessThanDate2(Date, Period.StartDate) || IsDate1MoreThanDate2(Date, Period.EndDate));
}
int GetPeriodLengthInDays(stPeriod Period1, bool IncludeEndDays = false)
{
	return GetDifferenceBetween2DatesByDays(Period1.StartDate, Period1.EndDate, IncludeEndDays);
}
int GetOverlapDays(stPeriod Period1, stPeriod Period2)
{
	int OverlapDays = 0;
	if (IsOverLapPeriods(Period1, Period2))
	{
		...
		{
			stDate CurrentDate = Period2.StartDate;
			while ((IsDate1LessThanDate2(CurrentDate, Period1.EndDate) && IsDate1LessThanDate2(CurrentDate, Period2.EndDate)))
			{
				IncreasDateByOneDay(CurrentDate);
				OverlapDays++;
			}
		}
		else if (IsDateInPeriod(Period2, Period1.StartDate))
		{
			stDate CurrentDate = Period1.StartDate;
			while ((IsDate1LessThanDate2(CurrentDate, Period1.EndDate) && IsDate1LessThanDate2(CurrentDate, Period2.EndDate)))
			{
				IncreasDateByOneDay(CurrentDate);
				OverlapDays++;
	...
	// If we reached here, that because Two Periods are not Overlap.
	cout << "Two Periods are not overlap(Note: Function : GetOverlapDays).\n ";
	return 0;
}
...
stDate StringToStDate(string DateString)
{
	stDate Date;
...
void PrintFormatDate(string StringDate, eFormatDate FormatDate = All)
{
	stDate Date = StringToStDate(StringDate);
	// NOTE : DMY = Day Month Year, slash = /  dash = -   ...etc
	if (FormatDate == ByName)
	{
		cout << "Day:" << Date.Day << ", Month:" << Date.Month << ", Year:" << Date.Year << endl;
	}
	else if (FormatDate == YDMslash)
	{
		cout << Date.Year << "/" << Date.Day << "/" << Date.Month << endl;
	}
	else if (FormatDate == MDYslash)
	{
		cout << Date.Month << "/" << Date.Day << "/" << Date.Year << endl;
	}
	else if (FormatDate == MDYdash)
	{
		cout << Date.Month << "-" << Date.Day << "-" << Date.Year << endl;
	}
	else if (FormatDate == MDYdash)
